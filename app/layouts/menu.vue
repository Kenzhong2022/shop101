    ...
    <div class="w-50 overflow-hidden">
        <el-col>
            ...
            >
                ...
                <template v-for="firstLevelMenu in menuListOOTD" :key="firstLevelMenu.index">
                    ...
                    >
                        ...
                        >
                            ...
                            <el-sub-menu
                                v-if="secondLevelMenu.children && secondLevelMenu.children.length"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <el-sub-menu
                                        v-if="
                                            thirdLevelMenu.children &&
                                            thirdLevelMenu.children.length
                                        "
...
<script lang="ts" setup>
    ...
    const handleMenuClick = (menuItem: MenuItem) => {
        ...
        // 检查路径是否以'/'开头，如果不是则添加
        const routePath = menuItem.path.startsWith("/") ? menuItem.path : `/${menuItem.path}`;

        // 执行路由跳转
        router.push(routePath).catch((err) => {
            // 如果路由不存在，提供友好提示
            if (err.name === "NavigationFailure") {
                console.warn(`路由不存在: ${routePath}，请检查是否需要创建对应的页面文件`);
            }
        });
    };
